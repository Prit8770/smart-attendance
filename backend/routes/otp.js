...
// Helper to resolve valid faculty table ID to satisfy foreign key constraint
...
router.get('/today', authenticateJWT, requireAdmin, async (req, res) => {
  try {
    ...
    // Select OTPs generated by this faculty member for the target date
    const { data: otps, error } = await supabase.from('otp')
      .select('*')
      .or(`generated_by.eq.${req.user.id},generated_by.eq.${facultyId}`)
      .eq('date', today);
...
router.get('/active', authenticateJWT, requireAdmin, async (req, res) => {
  try {
    let facultyId = req.user.faculty_id || req.user.id;
    const { data: facRow } = await supabase.from('faculty')
      .select('id')
      .or(`id.eq.${facultyId},email.eq.${req.user.email || ''},username.eq.${req.user.email || ''}`)
      .maybeSingle();
    if (facRow && facRow.id) facultyId = facRow.id;
...
router.post('/generate', authenticateJWT, requireAdmin, async (req, res) => {
  try {
    ...
    const facultyId = await resolveValidFacultyId(req.user);
    if (!facultyId) {
      return res.status(400).json({ error: 'No valid faculty account found to associate with this OTP session.' });
    }
